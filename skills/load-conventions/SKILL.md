...
Given the project's stack (from `infer-conventions`) and what the change touches, load the matching conventions packs from the open-review repo (`skills/conventions/<name>` — e.g. `laravel`, `react`, `inertia`).
...
Precedence: the project's documented conventions (what `infer-conventions` mapped) override packs; packs override general merits. Where the project documents a topic, the pack stays out of it.

Dual-use: this is the standard whether you're reviewing the code or writing it.
